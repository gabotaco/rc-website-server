...
export default class Payout extends Entity {
		...
		if (req.body.company === 'pigs') {
			...
			var NewDeadline = function (MemberDetails) {
				...
				if (D3 >= -45 * 24 * 60 * 60 * 1000) {
					...
					if (Math.abs(voucherAmount) > 60000) {
						//turnin in a lot
						...
						Math.abs(voucherAmount) <= 60000 &&
						Math.abs(voucherAmount) > 30000
					) {
						CurrentDeadline.setDate(
		...
		this.db.members
			...
			.then(memberDetails => {
				if (req.body.company === 'pigs') {
					...
					oldProgress = Math.floor(
						(memberDetails.pigs.vouchers / rankVouchers) * 100
					);
					if (rankVouchers != Infinity) {
						newProgress = Math.floor(
							((memberDetails.pigs.vouchers + voucherAmount) / rankVouchers) *
								100
						);
					} else {
						newProgress = 100;
					...
					oldProgress = Math.floor(
						(memberDetails.rts.vouchers / rankVouchers) * 100
					);
					if (rankVouchers != Infinity) {
						newProgress = Math.floor(
							((memberDetails.rts.vouchers + voucherAmount) / rankVouchers) *
								100
						);
					} else {
						newProgress = 100;
				...
				const newDeadline = NewDeadline(memberDetails)
					.toISOString()
					.split('T')[0];
	...
	confirm = async (req, res) => {
		if (!req.body.payout_id || !this.payouts[req.body.payout_id])
			return this.errorResponse(res);

		const payout = this.payouts[req.body.payout_id];

		this.db.sequelize.transaction(async t => {
			const manager = await this.db.managers.findByPk(payout.managerID, {
				transaction: t
			});
			...
			if (payout.company === 'pigs') {
				var company = await this.db.pigs.findOne({
					where: { member_id: payout.memberID },
					transaction: t
				});
			} else {
				var company = await this.db.rts.findOne({
					where: { member_id: payout.memberID },
					transaction: t
				});
			}
			...
			const member = await this.db.members.findByPk(payout.memberID, {
				transaction: t
			});
			member.deadline = this.db.sequelize.escape(payout.deadline).slice(1, -1);
			member.last_turnin = this.db.sequelize
				.escape(payout.currentDate)
				.slice(1, -1);
			await member.save({ transaction: t });

			if (payout.do_rank) {
				const serverID =
					payout.company == 'rts' ? '447157938390433792' : '487285826544205845';
	...
	getDetails = async (req, res) => {
		if (!parseInt(req.query.vouchers) || !req.query.company)
			return this.errorResponse(res);
